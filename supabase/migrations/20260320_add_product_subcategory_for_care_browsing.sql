...
where category in ('Hair', 'body_hair', '샴푸', '트리트먼트', '헤어에센스')
  and (
...
update products
set subcategory = 'Frizzy Hair'
where category in ('Hair', 'body_hair', '샴푸', '트리트먼트', '헤어에센스')
  and (
    name ilike '%곱슬%' or name ilike '%frizz%' or
    name ilike '%스무딩%' or name ilike '%smoothing%'
  );

update products
set subcategory = 'General'
where category in ('Hair', '샴푸', '트리트먼트', '헤어에센스')
  and subcategory is null;
...
where category in ('Body', 'body_hair', '바디로션', '바디워시', '핸드크림')
  and (
...
update products
set subcategory = 'Dry Heel'
where category in ('Body', 'body_hair', '바디로션', '바디워시', '핸드크림')
  and (
    name ilike '%발뒤꿈치%' or name ilike '%heel%' or
    name ilike '%풋크림%' or name ilike '%foot cream%' or
    name ilike '%foot%' or name ilike '%callus%' or
    name ilike '%발%'
  );

update products
set subcategory = 'General'
where category in ('Body', '바디로션', '바디워시', '핸드크림')
  and subcategory is null;
